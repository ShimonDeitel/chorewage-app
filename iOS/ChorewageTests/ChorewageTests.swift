import XCTest
@testable import Chorewage

@MainActor
final class ChorewageTests: XCTestCase {
    ...
    func testCannotAddMoreAtLimitWhenFree() {
        let store = Store()
        store.isPro = false
        store.items = (0..<Store.freeLimit).map { _ in Payout() }
    ...
    func testProUsersAlwaysCanAddMore() {
        let store = Store()
        store.isPro = true
        store.items = (0..<(Store.freeLimit + 10)).map { _ in Payout() }
        XCTAssertTrue(store.canAddMore)
    }
